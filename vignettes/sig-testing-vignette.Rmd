...
vignette: >
  %\VignetteIndexEntry{Significance Testing}
...
To add significance testing to a table, compute the table with `tab()` and then apply `add_sig()`:
...
# Access significance results (extracted during materialization)
df <- as.data.frame(result)
sig_results <- attr(df, "significance")
```

### Applying significance across a table

For workflows that compute multiple statistics or more complex tables, `add_sig_all()` can be used to apply significance testing across relevant cells in one step.

```{r add_sig_all_example}
result_all <- tab(data, labelledordinal, labelledcategorical,
                  statistic = "column_pct") %>%
  add_sig_all(test = "auto", level = 0.05)

attr(as.data.frame(result_all), "significance")
```
...
tab(data, labelledordinal, labelledcategorical) %>%
...
# Compare to a specific column by label
tab(data, labelledordinal, labelledcategorical) %>%
...
-   **levels**: Matrix showing significance status for each cell
    ...
    -   `""` (empty string): No significant difference (p >= level)
...
sig <- attr(as.data.frame(result), "significance")
...
If you call `add_sig(test = "auto")` on a weighted table, surveydatar will automatically select weighted tests where available (e.g., `z_test_weighted` for `column_pct`, `t_test_weighted` for `mean`).
