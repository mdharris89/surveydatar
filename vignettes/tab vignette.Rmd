...
## Advanced Feature: Gates and Orthogonal Tables

Gates control which cells are calculated based on row/column metadata. For example, to create a brand-attribute matrix where each row/column is a brand x frequency combination, you can use:

```{r brand_attr_matrix, eval=FALSE}
# Brand–attribute style matrix using value labels
tab(survey_data,
  rows = calc_if(no_mismatch("ival"),
                 response_match(c("Daily", "Weekly", "Monthly"), "A2", mode = "value")),
  cols = response_match(c("Daily", "Weekly", "Monthly"), "A2", mode = "value"),
  statistic = "count",
  sum_by = "ival",
  show_base = FALSE, show_row_nets = FALSE, show_col_nets = FALSE
)
```

What happens step-by-step:

- First, calc_if(no_mismatch("ival")) creates an orthogonal table where only cells on the matching row/col value are computed; off-diagonals are blank/zero.
- Then, sum_by = "ival" collapses those duplicates by the shared value label, leaving one row/column per unique value and preserving the gated (diagonal) structure.

Note: This is an advanced, unusually imperative pattern in the package. A simpler, more declarative syntax for this use case will likely become the recommended approach.
...
Sort your results by values or labels:

```{r sorting}
# Sort rows by NET column values (descending)
sorted_result <- sort_tab(comprehensive_result, rows = "NET")

# Sort columns alphabetically  
alpha_sorted <- sort_tab(comprehensive_result, 
                         cols = list(by = "label", order = "asc"))

# Sort with natural ordering (handles numbers properly)
natural_sorted <- sort_tab(comprehensive_result,
                          rows = list(by = "label", natural = TRUE))

# Restore original order
restored <- sort_tab(natural_sorted, rows = "original", cols = "original")
```
