---
title: "Creating Cross-Tabulations with surveydatar::tab()"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Creating Cross-Tabulations with surveydatar::tab()}
...
### Brand questions: skip-logic vs. awareness-based bases

In market research, brand questions are often only asked to respondents who are aware of a brand. In SPSS exports this is commonly represented as:

- **`NA`**: respondent was *not asked* about the brand
- **`0`**: respondent was asked but did *not* select the brand
- **`1`**: respondent was asked and *did* select the brand

For `statistic = "column_pct"`, `tab()` treats **eligibility (base)** and **membership (numerator)** separately:

- **Membership (m)**: who meets the row condition (e.g., “considered Brand A”)
- **Universe (u)**: who is eligible to be counted in the denominator (e.g., “aware of Brand A”)

By default, `tab()` infers eligibility from missingness: if a row expression yields `NA` for a respondent, they are treated as *not eligible* for that row’s base.

Sometimes, though, respondents are asked about *all* brands, and you want to compute consideration/perception **among those aware** at analysis time. You can do this in a single `tab()` call using `among(x, universe)`, which attaches an explicit universe to the row definition.

```{r brand_skip_logic_vs_awareness_base}
set.seed(1)
n <- 300

brand_data <- data.frame(
  region = factor(sample(c("North", "South", "East", "West"), n, replace = TRUE)),
  aware_a = rbinom(n, 1, 0.70),
  aware_b = rbinom(n, 1, 0.55),
  aware_c = rbinom(n, 1, 0.35)
)

# Scenario 1: asked only if aware (NA = not asked)
brand_data$consider_a_skip <- ifelse(brand_data$aware_a == 1, rbinom(n, 1, 0.40), NA_integer_)
brand_data$consider_b_skip <- ifelse(brand_data$aware_b == 1, rbinom(n, 1, 0.30), NA_integer_)
brand_data$consider_c_skip <- ifelse(brand_data$aware_c == 1, rbinom(n, 1, 0.25), NA_integer_)

tab(
  create_survey_data(brand_data),
  rows = rows_list(
    "Brand A (asked-if-aware)" = consider_a_skip == 1,
    "Brand B (asked-if-aware)" = consider_b_skip == 1,
    "Brand C (asked-if-aware)" = consider_c_skip == 1
  ),
  cols = region,
  statistic = "column_pct"
)

# Scenario 2: asked everyone (0/1 for all), but base should be "aware of the brand"
brand_data$consider_a_all <- rbinom(n, 1, 0.40)
brand_data$consider_b_all <- rbinom(n, 1, 0.30)
brand_data$consider_c_all <- rbinom(n, 1, 0.25)

tab(
  create_survey_data(brand_data),
  rows = rows_list(
    "Brand A (among aware)" = among(consider_a_all == 1, aware_a == 1),
    "Brand B (among aware)" = among(consider_b_all == 1, aware_b == 1),
    "Brand C (among aware)" = among(consider_c_all == 1, aware_c == 1)
  ),
  cols = region,
  statistic = "column_pct"
)
...
-   row_m: Membership (1 if that respondent matches the row condition, 0 otherwise)
-   row_u: Row eligibility (1 if respondent is in-base for denominators for this row, 0 otherwise)
-   col_m: Membership (1 if that respondent matches the column condition, 0 otherwise)
-   col_u: Column eligibility/base context (1 if respondent is in-base for denominators for this column, 0 otherwise)
...
`tab()` also computes **exposure** for each row and column, used only for `hide_empty`.

- **Row exposure** is the marginal mass of the row membership under the table base:  
  `row_exposure = sum(base_array * row_m)`
- **Column exposure** is the marginal mass of the column membership under the table base:  
  `col_exposure = sum(base_array * col_m)`

“Marginal” here means one-dimensional: it ignores how mass is distributed across the other axis. That’s exactly what `hide_empty` needs, because it makes a row/column-level decision (“does this row/column have any mass at all?”), not a per-cell decision.
...
-   For column %: sum(final) / sum(base \* col_m \* row_u) × 100
...
median_stat <- function() {
  create_statistic(
    ...
    processor = function(base_array, row_m, row_u, col_m, col_u, values = NULL, ...) {
      ...
      cell_membership <- base_array * row_m * col_m
    ...
    summary_row_calculator = function(arrays, base_array) {
      # Union summary: eligible if asked at least one; member if meets at least one
      m_mat <- do.call(cbind, lapply(arrays, `[[`, "m"))
      u_mat <- do.call(cbind, lapply(arrays, `[[`, "u"))
      structure(
        list(
          m = as.numeric(rowSums(m_mat) > 0),
          u = as.numeric(rowSums(u_mat) > 0)
        ),
        class = "tab_arrays"
      )
    },
...
ci_width_stat <- function() {
  create_statistic(
    ...
    processor = function(base_array, row_m, row_u, col_m, col_u, values = NULL, ...) {
      ...
      denominator <- sum(base_array * col_m * row_u)  # Column total (among eligible)
...
sig_info <- attr(as.data.frame(result_with_sig), "significance")
print(result_with_sig)  # Prints with significance indicators
```

Available tests include `z_test_proportions`, `t_test`, `chi_square`, and `mann_whitney`. Use `"auto"` for automatic test selection when supported (otherwise it errors with guidance).
...
# Keep only specific labels (equivalent to negating a label-set matcher)
result <- tab(survey_data, satisfaction, region) %>%
  hide_rows_except("Very satisfied|Satisfied")
```
...
result <- tab(survey_data, satisfaction, region) %>%
  arrange_rows(.order_fn = function(cell) {
    ...
    cell$specification$meta$display_order %||% 0
  })
...
top_rows <- head(result$layout$row_labels, 5)
result <- select_rows(result, top_rows)
...
result <- tab(survey_data, gender, region, 
              statistic = "mean", values = "income") %>%
  derive(index_vs(base_col = "Avg", multiplier = 1))
...
result <- tab(survey_data, satisfaction, region,
  ...
  hide_cols_except("North|South|.* - .*|Index") %>%
...
## Running many tables with multi_tab()

In reporting workflows, it is common to repeat the same table across multiple segments (for example, one table per region or country). `multi_tab()` helps generate these in one call and combine them into a single result.

### Split into column blocks

```{r multi_tab_cols}
# Create one table per region, combined as additional column blocks
multi_tab(
  survey_data,
  rows = satisfaction,
  cols = gender,
...
The convenience wrapper `multi_tab_cols()` is equivalent:

```{r multi_tab_cols_wrapper}
multi_tab_cols(
  survey_data,
  rows = satisfaction,
  cols = gender,
...
You can also define groups explicitly using a named list of filters:

```{r multi_tab_custom_groups}
multi_tab(
  survey_data,
  rows = satisfaction,
  cols = gender,
