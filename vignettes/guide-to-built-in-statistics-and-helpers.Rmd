---
title: "Guide to built-in statistics and helpers"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Guide to built-in statistics and helpers}
...
# Minimal example dataset used by the code snippets below
data <- create_survey_data(get_big_test_dat(200))
```
...
**Note**: For grid questions (variable × value matrices), consider using `pivot_to_grid()` as an alternative. It provides a cleaner approach to reshaping grid-style data. See the tab vignette for details.
...
### pivot_to_grid (pivot_to_grid)

Reshapes tables by extracting semantic dimensions from cells. Particularly useful for grid questions.

``` r
# Grid question with variables × values
tab(data, usage_a, statistic = "row_pct") %>%
  pivot_to_grid()
# Creates a matrix with tools as rows, frequency as columns

# Custom extractors
tab(data, brand_attrs_a, region) %>%
  pivot_to_grid(
...
## Macros and higher-level helpers

Macros transform the parameters you pass to `tab()` before any helpers are
parsed. They are handy when a table requires coordinated changes to multiple
arguments. Helpers such as `banner()` remain available for lower-level tasks.
